...
★★★ 4c단계 — E0 v2b (적합 36 으로 교란 제거) ★★★
  커밋 26a30fd. 분할: 54종목 → 적합 36 / 선택 18, 6개 층 전부 6:3.
  게이트: 1/5 (L1), 거부권 미발동, verdict stop. 표면 수치는 v2 와 동일.

  ★ L1 은 확증됐다 — 우연이 아니다.
      v1  표본내 적합36   복원 False
      v2  표본외 적합24   복원 True   (4트랙 중 2)
      v2b 표본외 적합36   복원 True   (4트랙 **전부**)
      홀함수 오차: v1 0.359/0.325/0.184 → v2 0.078 → v2b **~1e-15**
      복원된 형태 `I^5/f(S)^2` — 사실상 완벽한 홀함수.
    적합 집합 크기를 v1 과 같게 되돌려도 유지되므로, 표본 외 채점이 L1 을
    고쳤다는 것은 종목 수 교란 없이 확인됐다.

Ruling R26 (내가 세운 L5 이분법은 틀렸다 — 구현자가 옳다):
  나는 "L5 퇴행이 종목 수 탓인가 채점 규칙 탓인가"로 물었다. 구현자가 이 이분법을
  거부했고 근거가 맞다. 코드로 확인했다 — `_fit_track` 에서
  `backend.fit(X, y, w, names)` 는 **적합 데이터만** 보고, 채점(`_score_and_judge`)은
  그 뒤에 별도로 일어난다. 따라서 `any_candidate_recovered` 는 후보 목록에서
  나오고, 후보 목록은 채점 규칙이 손대기 전에 확정된다.
  **채점 규칙이 L5 를 깨뜨렸다는 것은 논리적으로 성립할 수 없다.** 채점은 무죄다.

  내가 놓친 잔여 교란도 구현자가 짚었다: 적합 집합의 **크기**를 36 으로 맞춰도
  **구성**은 못 맞춘다. v1 의 36종목 중 23개만 v2b 적합에 남고 13개는 select 로
  밀려나 새 13개로 대체됐다 — select 가 fit 과 서로소여야 하는 한 구조적으로
  불가피하다. 내가 제안한 대조군은 완전한 대조군이 아니었다.

  ★ 실측으로 드러난 것 — L5 파레토 프론트의 exp/log 포함 후보 수:
      v1  (적합36 원본구성)   **9 / 15**   복원 ✓
      v2  (적합24)              1 / 12     복원 ✗
      v2b (적합36 다른구성)     **0 / 13**  복원 ✗
    적합 종목 구성이 바뀌자 SR 이 아예 다른 함수 계열(Abs 기반 구간선형)로 갔다.
    → **L5 의 v1 복원은 종목 구성에 취약했다.** 그리고 L5 는 v1 에서 유일하게
      복원된 법칙이었다. 즉 v1 의 "1/5" 는 다른 36종목 추첨에서는 재현되지 않는
      결과 위에 서 있었다. 우리 자신의 기록을 약화시키는 방향의 발견이다.

Ruling R27 (L5 의 시드 민감도를 직접 잰다 — 싸고 결정적이다):
  위 관측은 seed 0 세 실행에서 나온 것이고, 세 실행은 적합 구성이 서로 다르다.
  "L5 복원이 종목 추첨 운이었는가"는 **L5 단독**(`--laws L5`)으로 시드를 바꿔
  돌리면 직접 잴 수 있다. 법칙 하나면 약 40분이므로 3개 시드에 2시간이다.
  전체 실행(4~5시간)을 반복할 이유가 없다.
  이것이 중요한 이유: L5 는 v1 에서 유일한 복원이었고, "1/5 이지 0/5 가 아니다"의
  유일한 근거였다. 그것이 추첨 운이면 우리는 그 숫자를 정정해야 한다.

--- 중단과 재개 (2026-09-06 밤 ~ 2026-09-08) ---

세션이 끊겨 작업이 L5 시드 1 완료 시점에서 멈춰 있었다. 2026-09-08 18:48 KST 에
상태를 점검해 재개했다. 멈춘 지점:
  - L5 시드 민감도: seed 1 까지만 실행됨 (seed 2·3 미실행)
  - shallow@700 / deeplob@700: 미실행
  - 미커밋 작업 552줄: run_e0.py, sd/e0/{runner,targets,teacher}.py + 테스트 3개
    (DeepLOB 을 E0 에 배선하던 중. `--teacher shallow|deeplob`, `--teacher-window`
     인자와 `_maybe_window` 추가. window<=1 이면 X 를 그대로 돌려주도록 방어돼
     있어 기존 교사 산출물이 바이트 단위로 보존된다고 주석에 명시)

★★★ Ruling R27 결과 — L5 복원은 종목 추첨에 좌우된다 (확인됨) ★★★
  L5 파레토 프론트에서 exp/log 항을 포함한 후보 수와 복원 여부:

      v1   seed 0, 적합 36 (원본 구성)    복원 **True**    9/15
      v2   seed 0, 적합 24                복원 False       1/12
      v2b  seed 0, 적합 36 (다른 구성)    복원 False       0/13
      L5   seed 1, 적합 36                복원 **True**   12/14

  **같은 설정(per_stratum=9, 적합 36)에서 시드만 0 → 1 로 바꾸니 복원이 뒤집혔다.**
  중간값이 없다 — SR 은 exp/log 계열을 거의 안 찾거나(0/13) 대부분 찾거나(12/14)
  둘 중 하나로 갈린다. 함수 계열 자체가 적합 종목 구성에 따라 통째로 바뀐다.

  함의: L5 는 v1 에서 **유일하게 복원된 법칙**이었다. 즉 E0 v1 의 "1/5 이지 0/5 가
  아니다"라는 숫자가 종목 추첨에 흔들리는 토대 위에 서 있었다. 이것은 우리 자신의
  기록을 약화시키는 방향의 발견이므로 그대로 남긴다.
  seed 2·3 을 마저 돌려 4개 점으로 확정한다 — 2개 점으로도 "흔들린다"는 결론은
  이미 나왔지만, 뒤집히는 비율을 재는 것은 별개의 값이다.

--- 물려받은 DeepLOB 배선 검증 (코디네이터 직접 수행) ---

에이전트가 세 번 연속 대기 상태로 멈춰(결과가 나왔는데 커밋도 다음 실행도 없음)
검증을 코디네이터가 직접 했다.

검증 1 — 배선이 기존 결과를 오염시키지 않는가: **통과.**
  `--teacher shallow` 로 seed 1 을 재현해 배선 이전 산출과 대조했다.
    배선 전 (runs/e0-20260905T161649Z)  복원=True any=True 후보=14 exp/log=12 행=1489
    배선 후 (runs/e0-20260908T100609Z)  복원=True any=True 후보=14 exp/log=12 행=1489
    파레토 프론트 표현식 목록 **동일(바이트 단위)**.
  주석이 "window<=1 이면 X 를 그대로 돌려준다"고 주장했는데, 그 주장이 산출물
  수준에서 확인됐다. 테스트 360 passed 만으로는 이것이 보장되지 않는다 —
  통과와 "이전과 같은 답"은 다른 명제다.

검증 2 — 전체 테스트: 360 passed, 9 deselected (341초, `-m "not slow"`).
  이전 커밋 시점 355 → 360, 신규 5개.

뮤테이션 1 — `_maybe_window` 의 `if window <= 1` 을 `if True` 로 바꿔
  윈도우가 조용히 무시되게 만듦: **잡힘.** 테스트 2개 실패.
    test_maybe_window_delegates_to_make_causal_windows_when_window_gt_one
    test_run_law_windows_teacher_input_before_subsample_reorder_and_respects_symbol_boundaries
  두 번째 테스트가 특히 값지다 — 윈도우를 **표집 이전에** 씌우는지까지 검사한다.
  표집 후 행렬에 윈도우를 씌우면 "바로 위 행"이 시간상 이웃이 아니라 무작위로
  뽑힌 다른 시각(다른 종목일 수도)의 행이므로 "시간 윈도우"라는 말이 거짓이 된다.
  원복 후 md5 대조 OK.

뮤테이션 2 — `run_e0.py` 의 `if args.teacher == "deeplob"` 을 `if False` 로 바꿔
  플래그를 줘도 조용히 shallow 가 쓰이게 만듦: **잡힘.**
    tests/test_run_e0_cli.py::test_teacher_deeplob_flag_actually_selects_scalardeeplob
  이 뮤테이션이 안 잡혔다면 `--teacher deeplob` 실행 결과가 실제로는 shallow 의
  결과인데 DeepLOB 의 결과로 보고될 뻔했다 — 교사 비교 실험 전체가 무의미해진다.
  원복 후 md5 대조 OK (run_e0.py, sd/e0/runner.py, sd/e0/teacher.py 전부).

  → 물려받은 코드 검증 완료. 커밋하고 실행에 들어간다.

★★★ L5 시드 민감도 — 4개 점 완성 (Ruling R27 결과) ★★★
  전부 `--laws L5 --per-stratum 9 --epochs 250 --sr-niterations 200` 동일, seed 만 다름.
  적합 36 / 선택 18, 행 317,437 로 모두 같다.

      seed 0   복원 **False**   exp/log  0/13   (= v2b)
      seed 1   복원 True        exp/log 12/14
      seed 2   복원 True        exp/log 11/15
      seed 3   복원 True        exp/log  4/13

  **4개 중 3개 복원. seed 0 이 예외다.**

  ★ 내 해석 정정 두 건:
  (1) "L5 의 v1 복원은 추첨 운이었다"(Ruling R27 도입부) 는 **과했다.** 4개 시드 중
      3개가 복원되므로, 운이었던 쪽은 v1 의 복원이 아니라 **v2b(seed 0)의 실패**다.
      L5 는 대체로 복원되는 법칙이고 seed 0 에서만 SR 이 함수 계열을 통째로 놓쳤다.
      → 원장 앞부분의 "v1 의 1/5 가 흔들리는 토대 위에 있었다"는 서술은 이 4개 점
        앞에서 성립하지 않는다. v1 의 L5 복원은 재현된다.
  (2) "중간값이 없다 — 0개 아니면 11~12개" 도 **성급했다.** seed 3 은 exp/log 후보가
      4개뿐인데도 복원됐다. 분포는 이분법이 아니라 연속적이고, 복원에 필요한 것은
      "프론트에 exp/log 가 많이 있는 것"이 아니라 "최고점수 후보가 그 형태인 것"이다.

  두 정정 모두 3개 점(seed 0 하나 + v1/v2/v2b)으로 성급히 일반화한 결과다.
  네 번째 점이 패턴을 뒤집었다. **점 3개로 곡선을 그리지 말 것.**

★★★★★ 최종 3자 비교 — 교사를 좋게 만들자 증류가 무너졌다 ★★★★★

  게이트:
    v2b shallow@250   1/5 [L1]  veto=False  stop
    shallow@700       1/5 [L1]  veto=False  stop     ← epoch 대조군
    deeplob@700       **0/5 []  veto=True**   stop

  대조군이 제 역할을 했다: epoch 250 → 700 은 5개 법칙의 recovered·any 를 **하나도**
  바꾸지 않았다. 따라서 deeplob@700 의 변화는 epoch 이 아니라 **교사 때문**이다.

  ★★ 그런데 원인은 "교사가 좋아져서"가 아니다. **교사가 과적합했기 때문이다.** ★★

  교사 R² (무차원 트랙):
              표본내(적합종목)        표본외(선택종목)
              shallow  deeplob       shallow   deeplob
    L1         +0.109   **+0.860**    -0.036   **-1.720**
    L2         +0.495   **+0.914**    +0.586   **+0.207**
    L3         +0.376   **+0.947**    +0.315   **-0.397**
    L4         +0.467   **+0.742**    +0.457   **+0.020**
    L5         +0.016   **+0.762**    -0.288  **-13.864**

  표본 내에서는 DeepLOB 이 압도적이다(0.74~0.95 vs 0.11~0.50). 표본 외에서는
  **전부 무너진다.** 음수 R² 는 평균을 찍느니만 못하다는 뜻이고, L5 의 -13.864 는
  파국이다. 윈도우 입력(19 → 16×19=304 차원)과 4.4배 파라미터로 적합 종목을
  외웠고 안 본 종목에 전혀 이식되지 않았다.

  → 게이트가 1/5 → 0/5 로 나빠진 것은 "더 좋은 교사가 증류를 해친다"가 아니라
    **"외운 교사가 증류를 해친다"** 이다. 후자는 당연한 결과다.
    후보 수도 전 법칙에서 줄었다(11→9, 14→13, 16→14, 12→11, 14→8) — 병목 z 가
    노이즈를 나르면 SR 이 탐색할 구조가 줄어든다. 4b 에서 관측된 z 붕괴
    (두 축 상관 0.95~0.97)와 같은 방향이다.

Ruling R28 (내 R24 는 표본 내 숫자에 근거한 잘못된 판단이었다):
  나는 4b 비교표를 근거로 "epochs 250 에서는 DeepLOB 이 더 나쁘니 700 으로 올려야
  공정하다"고 판정했다(R24). 그 비교표의 r2_path/r2_fill 은 **표본 내** 값이다 —
  두 교사에게 "완전히 같은 행·같은 가중치"를 주고 그 행에서 쟀다. 홀드아웃이 없었다.
  즉 "700 에서 DeepLOB 이 역전한다"는 관측 자체가 **더 큰 모델이 더 많이 외운 것**을
  본 것일 수 있다. 그 위에서 epoch 을 올린 내 결정은 과적합을 키우는 방향이었다.
  다행히 대조군(shallow@700)을 함께 돌린 덕분에 epoch 효과가 없음이 확인됐고,
  runner 에 이미 `r2_vs_real_y_select_symbols` 가 기록되고 있어 사후에 진상이 드러났다.

  ★ 이 프로젝트의 중심 교훈이 한 층 더 깊은 곳에서 반복됐다.
    우리는 **후보 선택**의 표본 내 채점을 고쳤다(v2, L1 복원). 그런데 **교사 훈련·
    평가**는 여전히 표본 내였다. 같은 함정이 파이프라인의 위층에 그대로 남아 있었고,
    교사를 키우자 그 함정이 터졌다.
    "표본 내에서 좋아 보이는 것을 고르면 노이즈를 고른다" — 후보에서도, 교사에서도.
