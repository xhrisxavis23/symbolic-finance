...
> **교사 모델 자체를 정본 백테스트에 넣어도 벌지 못한다(0914).** 딥러닝 교사는 안 본 종목에서 이익을 예측한 틱이 하나도 없고, 상위 5% 신호로 사면 체결당 약 −19bp로 증류 수식과 구별되지 않는다.
...
| [`0914`](0914/) 교사 자체의 경제성 | 09-14 | 교사를 수식으로 옮기지 않고 그대로 매수 신호로 써서 ① 라벨 기준 ② 정본 백테스트로 잼 | **교사 자체도 벌지 못한다.** 체결당 −19~−25bp, 0910 수식과 구별 안 됨 |
...
0911까지 교사는 R²로만 쟀다. 0914에서는 교사 점수를 수식으로 옮기지 않고 **그대로 매수 신호로 써서 정본 백테스트에 넣었다**
(`20260319`, 안 본 종목 411개, 사전등록 후 실행, vendor 프레임워크 무수정).

| 교사 | `score > 0` (손익분기) | 직전 100틱 상위 5% (Q95): 의사결정당 bp [본페로니] | 체결당 net / gross (bp) |
| --- | --- | --- | --- |
| DeepLOB 200만 행 | 거래 없음 | −0.797 [−1.012, −0.594] | −21.1 / +1.9 |
| DeepLOB 3일 | 거래 없음 | −0.673 [−0.883, −0.491] | −18.7 / +4.3 |
| 릿지 | 305결정 · 판정 불가 | −0.729 [−0.935, −0.532] | −19.7 / +3.3 |
| (비교) 0910 증류 수식 시드 2 | — | −0.607 [95% −0.731, −0.485] | −18.4 / +4.6 |
| (비교) 나이브 `book_imbalance` | — | −0.866 [95% −1.006, −0.736] | −25.3 / −2.3 |

- 교사 계약 12개(교사 3 × 규칙 4) 중 **수익 0**, 손실 9, 거래 부족 2, 판정 불가 1.
- DeepLOB 교사는 안 본 종목 385만 행에서 **한 번도 이익(`y_path > 0`)을 예측하지 않았다.** 예측 보정은 맞는다 — 가장 좋다고 매긴 0.1%도 실제로는 손실이다.
- 수수료 전에는 방향을 조금 맞히지만(체결당 +4bp) 왕복 23bp를 넘지 못한다. **교사와 증류 수식의 손익이 구별되지 않으므로 병목은 증류가 아니라 교사가 읽는 신호의 크기**다.
- 전문: [`0914/RESULTS-STAGE1.md`](0914/RESULTS-STAGE1.md) · [`0914/RESULTS-STAGE2.md`](0914/RESULTS-STAGE2.md)
...
│   ├── board/                진행보드 보고서 생성
│   └── symbolic_distillation_0911_report.html   (0914 절 포함)
│
├── 0914/                     교사 자체의 경제성 (R²가 아니라 손익으로)
│   ├── PREREG-TEACHER-ECON.md 사전등록 (부록 A: C1 정정)
│   ├── RESULTS-STAGE1.md · RESULTS-STAGE2.md · PROGRESS.md
│   ├── stage1/               저장된 예측으로 라벨 기준 경제성
│   └── stage2/               교사 점수 생성 · 주입(vendor 무수정) · 점검 C1′~C4 · 정본 재생
│
...
- **증류가 기여하는가**: 증류를 뺀 비교(절제 실험)를 본 실험 규모에서 돌린 적이 없다. 0914에서 교사 자체와 증류 수식의 손익은 구별되지 않았다.
- **계약에 맞춘 학습 목표**: 교사는 30초 즉시 매수·매도 라벨을 배웠다. 정본 계약 손익을 직접 겨냥하면 교사가 달라지는지 모른다(0914 결과가 가리키는 첫 질문).
- **봉인 날짜 성적**: `20260420` 이후는 열지 않았다.
- **3일 교사로 다시 증류한 수식의 성과**: 0914에서 3일 교사 자체가 정본 계약에서 벌지 못해 기대는 낮다.
- **체결확률 헤드 보정**: 0910에서 교사 검증 게이트가 실패한 원인.
- **시험일이 하루(3/19)뿐**이라 날짜 간 변동을 모른다.

남은 선택지(계약 손익에 맞춘 학습 목표, 3일 교사 재증류, 큰틱·고유동 종목으로 좁히기, MLPLOB 등 모델 교체, 학습 날짜 확대)는 [`EXPERIMENT-RECORD.md` §9](EXPERIMENT-RECORD.md)에 정리돼 있다.
